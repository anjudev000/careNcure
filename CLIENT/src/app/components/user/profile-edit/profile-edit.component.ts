...
export class ProfileEditComponent {
  ...
  constructor(private fb:FormBuilder,
    ...
    @Inject(MAT_DIALOG_DATA) public data: { userData: UserProfile }
      ...
      if (this.userdata.address && this.userdata.address.length > 0) {
    this.editForm = fb.group({
      ...
      address: this.fb.group({ // Define the address field as a nested FormGroup
        houseName: [this.userdata.address[0]?.houseName || ''],
        houseNumber: [this.userdata.address[0]?.houseNumber || ''],
